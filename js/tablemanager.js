...
SQL.TableManager.prototype.click = function (e) {
  ...
  while (target && target !== area && target !== document.body) {
    if (
      ...
      target.className.indexOf("table") !== -1
    ) {
      clickedOnTable = true;
      break;
    }
...
SQL.TableManager.prototype.showAddTableDialog = function () {
  ...
  this.owner.window.close = function () {
    ...
    if (
      self.adding &&
      self.pendingTablePosition &&
      !self.tableCreatedFromDialog
    ) {
      self.adding = false;
  ...
  this.owner.window.open(
    _("edittable"),
    this.dom.container,
    this.createPendingTable.bind(this)
  );
...
SQL.TableManager.prototype.createPendingTable = function () {
  ...
  var newtable = this.owner.addTable(
    name,
    this.pendingTablePosition.x,
    this.pendingTablePosition.y
  );
...
SQL.TableManager.prototype.paste = function () {
  ...
  var nameCounter = 1;
  while (
    this.owner.tables.some(function (t) {
      return t.getTitle() == newName;
    })
  ) {
...
SQL.TableManager.prototype.press = function (e) {
  ...
  if (ctrlKey && e.keyCode == 86) {
    ...
    if (
      this.owner.rowManager.clipboard &&
